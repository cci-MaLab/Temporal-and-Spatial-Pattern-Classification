...
def process_session(sess_value, labels_train, labels_test, amp_train, amp_test, auc_train, auc_test, frq_train, frq_test, 
    ...
    """
    This function processes the data for a given session (0 or 1) by filtering the train/test sets, selecting the feature set (AMP, AUC, FRQ),
    and training different models (XGBOOST, SVM, Neural Network) with and without Alp_Prev.
    
    Parameters:
    - sess_value (int): The session value to filter by (0 or 1)
    ...
    if sess_value is not None:
        ...
        condition_train = (labels_train[:, 4] == sess_value)
        condition_test = (labels_test[:, 4] == sess_value)
    ...
    for feat in feature_set:
        ...
        for model_name in models:
            ...
            if model_name == 'XGBOOST':
                ...
                store_results('D1vsD7.csv', dataset, sess_str+combination, 'XGBOOST', classify, feat, acc, pre, rec, f1)
            elif model_name == 'SVM':
                acc, pre, rec, f1 = SVM_clf(train, y_train_filtered, test, y_test_filtered)
                store_results('D1vsD7.csv', dataset, sess_str+combination, 'SVM', classify, feat, acc, pre, rec, f1)
            elif model_name == 'Neural Network':
                
                acc, pre, rec, f1 = Neural_model(train, y_train_filtered, test, y_test_filtered)
                store_results('D1vsD7.csv', dataset, sess_str+combination, 'Neural_Network', classify, feat, acc, pre, rec, f1)
...
def train_models(dataset, combination, pca_file, classify):
    ...
    process_session(0, labels_train, labels_test, amp_train, amp_test, auc_train, auc_test, frq_train, frq_test, 
                amp_train_prev, amp_test_prev, auc_train_prev, auc_test_prev, frq_train_prev, frq_test_prev, 
                y_train, y_test, dataset, combination, classify)

    process_session(1, labels_train, labels_test, amp_train, amp_test, auc_train, auc_test, frq_train, frq_test, 
                amp_train_prev, amp_test_prev, auc_train_prev, auc_test_prev, frq_train_prev, frq_test_prev, 
                y_train, y_test, dataset, combination, classify)

    process_session(None, labels_train, labels_test, amp_train, amp_test, auc_train, auc_test, frq_train, frq_test, 
                amp_train_prev, amp_test_prev, auc_train_prev, auc_test_prev, frq_train_prev, frq_test_prev, 
                y_train, y_test, dataset, combination, classify)
